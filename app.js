...
/* Demo accounts (employees, manager, HR, individual) */
const DEMO_ACCOUNTS = {
  employee: [
    { id:"u_emp_001", email:"employee@demo.com", password:"employee123", name:"John Doe",    role:"employee", teamId:"team_eng", managerId:"u_mgr_001" },
    { id:"u_emp_002", email:"lee@demo.com",      password:"lee123",      name:"Lee Wong",    role:"employee", teamId:"team_eng", managerId:"u_mgr_001" },
    { id:"u_emp_003", email:"maria@demo.com",    password:"maria123",    name:"Maria Silva", role:"employee", teamId:"team_eng", managerId:"u_mgr_001" },
  ],
  manager: [
    { id:"u_mgr_001", email:"manager@demo.com",  password:"manager123",  name:"Alicia Patel", role:"manager", teamId:"team_eng" }
  ],
  hr: [
    { id:"u_hr_001",  email:"hr@demo.com",       password:"hr123",       name:"Sara Ahmed",   role:"hr" }
  ],
  individual: [
    { id:"u_ind_001", email:"learner@demo.com",  password:"learn123",    name:"Alex Jordan",  role:"individual" }
  ]
};

const ORG_USERS = [
  ...DEMO_ACCOUNTS.employee,
  ...DEMO_ACCOUNTS.manager,
  ...DEMO_ACCOUNTS.hr,
  ...DEMO_ACCOUNTS.individual
];
...
/* ========== LOGIN HELPERS ========== */
function _findDemoAccount(role, email, password){
  const list = DEMO_ACCOUNTS[role] || [];
...
function initCompanyLoginPage(){
  ...
  loginForm.addEventListener("submit", (e) => {
    ...
    const account = _findDemoAccount(selectedRole, email, password);
    ...
    if(account){
      ...
      const hints = (DEMO_ACCOUNTS[selectedRole] || [])
        .map(u => `${u.email} / ${u.password}`)
        .join(" | ");
      loginMessage.textContent = `Invalid ${selectedRole} credentials. Try: ${hints}`;
    }
  });
}
...
function initIndividualLoginPage(){
  ...
  loginForm.addEventListener("submit", (e) => {
    ...
    const account = _findDemoAccount("individual", email, password);
    ...
    if(account){
      ...
      const hints = (DEMO_ACCOUNTS.individual || [])
        .map(u => `${u.email} / ${u.password}`)
        .join(" | ");
      loginMessage.textContent = `Invalid credentials. Try: ${hints}`;
    }
  });
}
  ...
  document.querySelectorAll("[data-action]").forEach(el => {
    el.addEventListener("click", async (e) => {
      ...
      switch(action){
        ...
        case "create-account":
          alert("This is a prototype. In a live system, this would take you to an account creation page.");
          break;
